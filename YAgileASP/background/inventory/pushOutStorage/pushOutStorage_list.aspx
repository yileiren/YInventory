<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="pushOutStorage_list.aspx.cs" Inherits="YAgileASP.background.inventory.pushOutStorage.pushOutStorage_list" %>
...
<head runat="server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        * \brief
        * 新增出库单。
        * 作者：董帅 创建时间：2012-9-13 13:13:12
        */
        function addPushOutStorage()
        {
            window.parent.popupsWindow("#popups", "新增出库单", 600, 165, "inventory/pushOutStorage/pushOutStorage_edit.aspx", "icon-add", true, true);
        }

        /*!
        * \brief
        * 修改出库单。
        * 作者：董帅 创建时间：2012-9-13 14:00:55
        */
        function editPushOutStorage()
        {
            //判断选中
            if ($("input:checked[type='checkbox'][name='chkInv']").length != 1)
            {
                alert("请选中要编辑的出库单，一次只能选择一个！");
                return;
            }

            window.parent.popupsWindow("#popups", "修改出库单", 600, 165, "inventory/pushOutStorage/pushOutStorage_edit.aspx?id=" + $("input:checked[type='checkbox'][name='chkInv']").eq(0).val(), "icon-edit", true, true);
        }

        /*!
        * \brief
        * 删除入库单。
        * 作者：董帅 创建时间：2012-9-7 23:13:55
        */
        function deletePutInStorages()
        {
            //判断选中
            if ($("input:checked[type='checkbox'][name='chkInv']").length > 0)
            {
                return confirm("确认要删除选中的入库单？");
            }
            else
            {
                alert("请选中要删除的入库单！");
...
<body style="width:100%;margin:0px;background-color:#EEF5FD;">
    ...
    <div region="north" border="true" style="height:28px;background-color:#EEF5FD">
        <div style="width:200px;margin-left:auto;margin-top:0px;margin-right:0px">
            <a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="javascript:addPushOutStorage();">新增</a>
            <a href="#" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="javascript:editPushOutStorage();">修改</a>
            <a id="A1" href="#" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" onclick="javascript:return deletePutInStorages();" runat="server" onserverclick="butDeleteItems_Click">删除</a>
        </div>
    </div>
            ...
            <div style="width:1120px;margin:0px auto">
                <table class="admintable">
                    <tr style="width:100%;height:30px">
                        ...
                        <th class="adminth" style="width:150px">客户</th>
                ...
                <ItemTemplate>
                    <tr style="width:100%;height:30px">
                        ...
                        <td class="admincls0" style="text-align:center">
                            <a href="pushOutStorage_detail.aspx?id=<%#Eval("id")%>" class="easyui-linkbutton" iconCls="icon-detail" plain="true" onclick="">明细</a>
                        </td>
                    </tr>
                </ItemTemplate>
